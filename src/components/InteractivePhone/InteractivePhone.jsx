...
import './InteractivePhone.css';
import img1 from '../../assets/images/img1.png'; // Use real photo for building
import testimonial1 from '../../assets/images/testimonial1.png'; // Profile pic
import unCover from '../../assets/images/uncover.svg'; // Building icon
...
export default function InteractivePhone() {
  ...
  useEffect(() => {
    ...
    const sequence = async () => {
      while (isMounted) {
        ...
        await controlsCursor.start({ y: 120, opacity: 0.7, transition: { duration: 1.0, ease: "easeOut" } });

        // 3. Cursor taps
        await controlsCursor.start({ scale: 0.7, transition: { duration: 0.15 } });
        await controlsCursor.start({ scale: 1, transition: { duration: 0.15 } });

        // 4. Modal slides up over the screen
        controlsModal.start({ y: "0%", transition: { type: "spring", damping: 25, stiffness: 200 } });
        await new Promise(r => setTimeout(r, 1000));

        // 5. Cursor moves down to tap "Pav F"
        await controlsCursor.start({ y: 235, transition: { duration: 0.8, ease: "easeOut" } });
        ...
        controlsCursor.start({ opacity: 0, transition: { duration: 0.3 } });
        await controlsModal.start({ y: "100%", transition: { duration: 0.5, ease: "easeInOut" } });

        // 8. Destination marker pops in
        await controlsDestMarker.start({ opacity: 1, scale: 1, transition: { type: "spring", stiffness: 300 } });
  ...
  // Complex jagged city street path
